...
Hazop is not limited personnel and equipment. It is useful all system including an abstruct system which consist of information only like a goal structured notation(GSN).
...
## ex.2
What is the difference between operation, process, practice, procedure and policy?
https://www.linkedin.com/pulse/what-difference-between-operation-process-practice-policy-alghanim

"Process: Is a sequence or layers of procedures/operations/Practice that is consisted of people, machines, materials, methods and governing policies that are used all together to design, manufacture and delivery a good or service (product).
Process is the big picture of the conversion of an input into an output through-out procedures.
Process is used as milestones of implementing the operating model of an organization in alliance with its strategy.

Policy: Is the list of rules, or framework, that govern the process.

Procedure/Practices: Is a series of steps to achieve a consistent result (output). Procedures can be also called a practice, such as Standard Operating Procedure, (SOP), a Good Manufacturing Practice (GMP) or a Business Best Practice. And sometimes they just call it routine.

Operation: Is an activity or activities performed on a good or service by a single machine, person or both. However, it is not so different from a “Practice” or a "procedure" neither in definition nor application."

試訳：

過程(process)：人、機械、材料、方法、および商品またはサービス（製品）の設計、製造、配達に一緒に使用される管理政策(policy)で構成する手順/操作/プラクティスの列(sequence)または層(layer)です。過程は、入力から出力への変換手順の全体像です。過程は、戦略と提携して組織の運用模型(model)を実装する一里塚(mile stone)として使用します。

政策(policy)：過程を管理する規則または枠組みの一覧です。

手順(procedure)/実践(practice)：一貫した結果（出力）を達成するための一連の手順です。 手順は、標準操作手順（SOP）、適正製造基準（GMP）、業務良実践などの実践とも呼びます。 時々単に繰り返し作業(routine)と呼びます。

操作(operation)：単一の機械、人、またはその両方によって財またはサービスに対して実行される1つまたは複数の活動です。 ただし、定義も適用も「実践」や「手順」とそれほど違いはありません。
...
# Q7 principle and approach 
What is the difference between principle and approach?

# ex1.
Principle vs Approach - What's the difference?
https://wikidiff.com/approach/principle

" principle is a fundamental assumption while approach is the act of drawing near; a coming or advancing near. "

試訳：

原則は基本的な仮定です。やり方(approach)は接近する行為です。 やってくる、または近づいています。

# ex2.
Approach, Principles, Method & Technique
https://www.professorjackrichards.com/approach-principles-method-technique/
